...
class SwordGame:
    def __init__(self, root):
        self.root = root
        self.root.title("Swordfighter AI: Demo Game")
        ...
        self.player_sword = self.canvas.create_line(
            0, 0, 0, 0,
            fill="silver",
            width=3
        )
        ...
        self.enemy_sword = self.canvas.create_line(
            0, 0, 0, 0,
            fill="silver",
            width=3
        )
        ...
        self.info = self.canvas.create_text(
            400,
            30,
            text=f"Opponent: {self.level_names[self.current_idx]}",
            fill="white",
            font=("Courier", 14)
        )
    ...
    def show_message(self, text, color):
        ...
        self.root.after(
            1000,
            lambda: self.canvas.itemconfig(self.msg, text="")
        )
    ...
    def move_player(self, dx):
        ...
        if not self.is_attacking:
            ...
            if coords[2] > 800:
                self.canvas.move(self.player, 800 - coords[2], 0)
    ...
    def enemy_lunge(self):
        ...
        if abs(e_x - p_x) < 40:
            ...
            self.brain.learn("counter", False)

        self.root.after(
            200,
            lambda: self.canvas.move(self.enemy, 60, 0)
        )
    ...
    def reset_combat(self):
        ...
        self.canvas.itemconfig(self.enemy, fill="#300500")
    ...
    def game_loop(self):
        ...
        if not self.is_attacking:
            ...
            if e_x > p_x + 130:
                self.canvas.move(self.enemy, -speed, 0)
    ...
    def shake_screen(self):
        ...
        for i in range(4):
            ...
            self.root.after(
                i * 15,
                lambda: self.canvas.move("all", 5, 0)
            )
    ...
    def cycle_difficulty(self, e):
        ...
        self.brain = SwordAI(
            self.level_names[self.current_idx]
        )
        ...
        print(
            f"[SYSTEM]: Difficulty changed to "
            f"{self.level_names[self.current_idx]}"
        )
